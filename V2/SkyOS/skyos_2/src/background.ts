...
if (fs.existsSync(process.env.APPDATA + "/Parallel 42/The Skypark/2/DEV.txt")) {
  if (process.env.WEBPACK_DEV_SERVER_URL) {
    UserData =
      process.env.APPDATA + "/Parallel 42/The Skypark DEV/2/Skypad_Debug";
  } else {
...
const reframe = () => {
  ...
  const newSize = [
    Math.ceil(w * zoom) + (bounds.width - contentBounds.width),
    Math.ceil(h * zoom) + (bounds.height - contentBounds.height),
  ];
  ...
  if (!isStowed) {
    ...
    try {
      fs.writeFileSync(
        ...
        JSON.stringify(persistedLocation),
        "utf-8"
      );
    } catch (e) {}
  }
...
function createWindow() {
  ...
  win.webContents.on("did-finish-load", () => {
    ...
    win.webContents.send(
      "asynchronous-reply",
      "framed",
      persistedLocation[2] == 0 ? false : true
    );
    //win.webContents.setZoomFactor(1);
    //win.webContents.setVisualZoomLevelLimits(1, 1);
    win.webContents.send("asynchronous-reply", "networks", getNetworks());

    if (args["debug-menu"]) win.webContents.openDevTools();
  });
  ...
  win.once("ready-to-show", () => {
    ...
    let inBounds = false;
    if (
      windowBounds.x > closestDisplay.bounds.x &&
      windowBounds.x < closestDisplay.bounds.x + closestDisplay.bounds.width
    ) {
      if (
        windowBounds.y > closestDisplay.bounds.y &&
        windowBounds.y < closestDisplay.bounds.y + closestDisplay.bounds.height
      ) {
        inBounds = true;
      }
    }
...
function argParser() {
  ...
  argv.forEach(function (arg) {
    ...
    const _arg = arg.match(ARGUMENT_SEPARATION_REGEX);
    _arg.splice(0, 1);
    ...
    const argValue =
      _arg[1] !== ""
        ? parseFloat(arg[1]).toString() === _arg[1]
          ? +_arg[1]
          : _arg[1]
        : true;
...
function startServer() {
  const PORT = args["server-port"] || 3756;
  server.listen(PORT, () => console.log(`Server listening on port: ${PORT}`));

  server.use(express.static("./resources/dist"));
  server.get("*", (req, res) => {
    const file = fs.readFileSync("./resources/dist/index.html", {
      encoding: "utf-8",
    });
    res.send(file);
  });
  ...
  switch (cmd[0]) {
    ...
    case "moving": {
      try {
        ...
        windowMoved =
          Math.abs(windowStartPosition.x - x) > 10 ||
          Math.abs(windowStartPosition.y - y) > 10;
        ...
        if (win.webContents.isLoading()) {
          ...
          if (win.resizable != param) {
            if (param) {
              ...
              var newWidth =
                closestDisplay.size.width - 100 - win.getPosition()[0];
              var newHeight = closestDisplay.size.height - 100;

              win.setSize(
                newWidth < 400 ? 400 : newWidth > 1900 ? 1900 : newWidth,
                newHeight < 400 ? 400 : newHeight
              );
              //ontop = true;
    ...
    case "analytics": {
      switch (cmd[1]) {
        ...
        case "event": {
          ...
          AnalyticsInstance.event(param.Category, param.Action, {
            evLabel: param.Label,
            evValue: param.Value,
            clientID: param.CID,
          })
