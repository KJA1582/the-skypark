...
import Apps, {
  ...
  StatusType,
  NavType,
} from "./sys/foundation/app_bundle";
...
Apps.forEach((app: AppInfo) => {
  if (!app.app_isexternal) {
    ...
    router.addRoute({
      ...
      component: () =>
        import("./apps/" + app.app_vendor + "/" + app.app_ident + "/load.vue"),
      props: {
        app: app,
        appName: app.app_vendor + "_" + app.app_ident,
      },
...
new Vue({
  ...
  methods: {
        ...
        try {
          ...
          if (loaded_state.config.onboarding) {
            Object.assign(
              ...
              loaded_state.config.onboarding
            );
          }
          ...
          if (this.config.ui.changelogShow) {
            ...
            if (
              changeLogLast.getTime() !=
              new Date(this.config.ui.changelogLast).getTime()
            ) {
    ...
    stateSave() {
      if (this.state.device.stateLoaded) {
        ...
        console.log(
          "Failed to save store_system. Trying to save before restore."
        );
      }
    },
    ...
    routeStateLoad() {
      ...
      if (loadedStore) {
        try {
          const loaded_state = JSON.parse(loadedStore);
          if (new Date().getTime() - loaded_state.last < 7.2e6) {
      ...
      switch (wsmsg.name[0]) {
        ...
        case "failsconnect": {
          if (this.$os.awaitsTransponderReboot) {
            if (
              this.$os.awaitsTransponderReboot.getTime() + 30000 <
              new Date().getTime()
            ) {
              //300000
          ...
          if (!this.$os.awaitsTransponderReboot) {
            this.$os.addNotification(
              new Notification({
                ...
                Message:
                  "Make sure the Transponder is running to access your contracts, progress data and other functions of the Skypad.",
              }),
              false
            );
          }
            ...
            case "restart": {
              ...
              this.$os.addNotification(
                new Notification({
                  ...
                  Message:
                    "Access to your contracts, progress data and other functions of the Skypad will resume shortly.",
                }),
                false
              );
              break;
          ...
          switch (wsmsg.name[1]) {
            case "get": {
              this.state.services.userProgress = {
                ReliabilityUnlock: new Date(
                  wsmsg.payload.ReliabilityUnlock
                ).getTime(),
            ...
            case "add": {
              this.$os.addNotification(
                new Notification({
                  ...
                  Expire: wsmsg.payload.Expire
                    ? new Date(wsmsg.payload.Expire)
                    : null,
        ...
        case "eventbus": {
          switch (wsmsg.name[1]) {
            case "meta": {
              ...
              this.state.services.simulator.live =
                (Eljs.round(svc_loc.Lat, 3) != 0 &&
                  Eljs.round(svc_loc.Lon, 3) != 0) ||
                Math.round(svc_loc.GS) > 70;
              ...
              wsmsg.payload.forEach((pl: any) => {
                switch (pl.Type) {
                  case "Autopilot": {
                    ...
                    this.state.services.simulator.autopilot.HdgOn =
                      pl.Params.HdgOn;
    ...
    unlock() {
      const history = this.$os
        .getAppAllHistory()
        .filter(
          (x) => x.app_type != AppType.SLEEP && x.app_type != AppType.LOCKED
        );
      if (
        history.length > 1 &&
        history[history.length - 1] != this.$os.activeApp
      ) {
        const lastApp = history[history.length - 1];
        this.$router.push({
          name: lastApp.app_vendor + "_" + lastApp.app_ident,
        });
      } else {
  ...
  beforeMount() {
    if (navigator.userAgent.match(/(AppleWebKit)/i)) {
      this.state.device.isAppleWebkit = true;
    }
    ...
    Apps.forEach((app: AppInfo) => {
      ...
      if (app.app_serviced) {
        app.app_service = new (require("@/apps/" +
          app.app_vendor +
          "/" +
          app.app_ident +
          "/service.ts").default)(this, app);
      }
    });
  },
  mounted() {
